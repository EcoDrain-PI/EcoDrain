...
function obterDadosZO() {
    fetch(`/zonas/zonaOeste`, { cache: 'no-store' }).then(function (response) {
        if (response.ok) {
...
function plotarGrafico(dados, idBueiro) {
    ...
    const ctx = document.getElementById('zonaOestePrincipal').getContext('2d');
    ...
    fetch(`/medidas/tempo-real/${idBueiro}`, { cache: 'no-store' }).then(function (response) {
        if (response.ok) {
            response.json().then(function (novoRegistro) {
                obterDadosZO(idBueiro);  // Atualiza os dados do gráfico
