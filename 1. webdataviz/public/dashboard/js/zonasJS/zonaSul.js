...
function obterDadosZS() {
  fetch(`/zonas/zonaSul`, { cache: 'no-store' }).then(function (response) {
    if (response.ok) {
...
function plotarGrafico(dados, idBueiro) {
  ...
  const ctx = document.getElementById('zonaSulPrincipal').getContext('2d');
  ...
  fetch(`/medidas/tempo-real/${idBueiro}`, { cache: 'no-store' }).then(function (response) {
    if (response.ok) {
      response.json().then(function (novoRegistro) {
        obterDadosZS(idBueiro);  // Atualiza os dados do gráfico
